stream_pull_model <- function(request) {
    ...
    buffer <- list(status = glue::glue("Starting pull of {request_body$data$name}"))
...
stream_pull_model_callback <- function(x) {
    ...
    jsonlite::stream_in(
        ...
        handler = function(chunk) {
            # container <<- c(container, list(chunk))
            stream_pull_model_chunk_callback(chunk)
        }
    )
...
stream_pull_model_chunk_callback <- function(chunk, progress_bar = NULL) {
